...
class CreateAgItems extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        ...
        foreach ($aggregates as $key => $value) {
            ...
            $ag_groups_id = $ag_groups_ids[$key];

            echo $aggregate . ' : ' . $compte_scf . "\n";
            // AgItem::create(['item_name' => $aggregate, 'compte_scf' => $compte_scf]);
            DB::insert('insert into ag_items (item_name,status,compte_scf,ag_groups_id ) values ( ?,?,?,?)', [$aggregate, 1, $compte_scf, $ag_groups_id]);
        }
